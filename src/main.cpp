...
Bot bot1;                                //Непосредственно сами кнопки
Bot bot2;
Bot bot3;
...
#endif
  bot1.pin = 13;                          //Назначаем пины кнопок
  bot2.pin = 12;
  bot3.pin = 10;

  pinMode(bot1.pin, INPUT_PULLUP);        //Устанавливаем пины кнопок на чтение
  pinMode(bot2.pin, INPUT_PULLUP);
...
void loop() {
  ...
  if (clickBotton(bot1)) {                //Опрашиваем кнопку1 на нажатие
